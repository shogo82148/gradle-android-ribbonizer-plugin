...
'''.trim())
    ...
    def "getAdaptiveIcons"() {
        setup:
        def file = File.createTempFile("ic_launcher", ".xml")
        file.deleteOnExit()
        file.write('''
<?xml version="1.0" encoding="utf-8"?>
...
'''.trim())
        ...
        expect:
        Resources.getAdaptiveIcons(file).containsAll(["@mipmap/ic_launcher_foreground"])
    }

    def "getAdaptiveIcons with vector image"() {
        setup:
        def file = File.createTempFile("ic_launcher", ".xml")
        file.deleteOnExit()

        // ic_launcher.xml may be a vector resource.
        file.write('''
...
'''.trim())
        ...
        expect:
        Resources.getAdaptiveIcons(file).containsAll([])
    }
}
